...
$(function() {
    ...
    $('#add-product').on('click', function() {
        const productId = $('#modal-produit').val();
        const productText = $('#modal-produit option:selected').text();
        const prix = parseFloat($('#modal-prix').val()) || 0;
        const quantite = parseFloat($('#modal-quantite').val()) || 0;
        const stockDisponible = parseFloat($('#modal-stock-disponible').val()) || 0;
        ...
        if (quantite > stockDisponible) {
            toastr.error('La quantité demandée dépasse le stock disponible (' + stockDisponible + ')');
            return;
        }
        ...
        $('#view-produits tr').each(function() {
            if ($(this).find('.product-id').val() == productId) {
                existingRow = $(this);
                return false;
            }
        });
        ...
        if (existingRow) {
            toastr.error('Ce produit est déjà dans la liste');
            return;
        }
        ...
        const newRow = `
            <tr class="new-row">
                <td>
                    ...
                    <small class="stock-info text-muted">Stock: ${stockDisponible}</small>
                </td>
                <td>
                    <input type="number" class="form-control quantite-input" value="${quantite}" min="0.01" step="0.01" max="${stockDisponible}">
                </td>
                <td>
                    <input type="number" class="form-control prix-input" value="${prix}" min="0.01" step="0.01">
                </td>
                <td class="montant-cell">${montant.toFixed(2)} BIF</td>
                <td>
        ...
        // Refresh Select2 to exclude the newly added product
        $('.select2-modal-products').select2('destroy').select2({
            placeholder: 'Rechercher un produit...',
            allowClear: true,
            dropdownParent: $('#view-modal'),
            ajax: {
    ...
    $(document).on('click', '.remove-product', function() {
        ...
        // Refresh Select2 to include the removed product
        $('.select2-modal-products').select2('destroy').select2({
            placeholder: 'Rechercher un produit...',
            allowClear: true,
            dropdownParent: $('#view-modal'),
    ...
    // Handle quantity and price changes in the table
    $(document).on('input', '.quantite-input, .prix-input', function() {
        ...
        const stockMax = parseFloat(row.find('.quantite-input').attr('max')) || 0;
        
        // Validate quantity against stock
        if (quantite > stockMax) {
            toastr.warning('Quantité limitée au stock disponible: ' + stockMax);
            row.find('.quantite-input').val(stockMax);
            return;
        }
        
        const montant = quantite * prix;
        row.find('.montant-cell').text(montant.toFixed(2) + ' BIF');
